        ...
        while (scanner.hasNextLine()) {
            ...
            while (lscanner.hasNext()) {
                ...
                if (value.equals("A")) {
                    ...
                    // Try to get the values, but if there is an exception, just continue to use the null values
                    try {
                		make = lscanner.next();
                ...
                // Else if deleting
                    ...
                    // Use scanner to get values of make and year
                    String make = lscanner.next();
                    int year = Integer.valueOf(lscanner.next());
                    list.reset();
                    // Iterate through the list
                    for (int i = 0; i < list.size(); i++) {
                        if (list.hasNext()) {
                            Car currentCar = (Car) list.next();
                            // If the make and the year are the same, remove the car
